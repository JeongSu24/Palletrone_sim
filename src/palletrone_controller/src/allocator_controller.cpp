...
public:
  ...
  AllocatorController() : rclcpp::Node("allocator_controller")
  {
    ...
    this->declare_parameter("pcx", 0.0);
    this->declare_parameter("pcy", 0.0);
    this->declare_parameter("pcz", 0.0);

    Pc_(0) = this->get_parameter("pcx").as_double();
    Pc_(1) = this->get_parameter("pcy").as_double();
    Pc_(2) = this->get_parameter("pcz").as_double();

    RCLCPP_INFO(this->get_logger(), "allocator_controller params: Pc=[%.4f %.4f %.4f]",
                Pc_(0), Pc_(1), Pc_(2));
  }
